...
def calc_p_at_10(results, rel_doc):
    ...
    retrieved_num = min(len(results), 10)
    ...
    p_at_10 = ret_rel_num / retrieved_num
    return p_at_10
...
def calc_r_precision(results, rel_doc, num_rel_doc):
    ...
    r_precision = correct_num / num_rel_doc
    return r_precision
...
# calculate the bpref
def calc_bpref(common_doc, query_result, rel_length, non_rel, ordered_docs):
    bpref = 0
    # iterate over the relevant retrieved documents
    for doc in common_doc:
        # Calculate the bpref for each relevant retrieved doc
        bpref += 1 - first_Rank(query_result, doc, non_rel, rel_length, ordered_docs) / rel_length
    return bpref/rel_length


# Calculate the number of non-relevant docs before a given document
def first_Rank(query_result, doc, un_rel, rel_length, ordered_docs):
...
def calc_evaluation_values(results_file, qrels_file):
    ...
    for query_id, results in query_results.items():
        ...
        # calc values for the current query
        ...
        bpref = calc_bpref(common_docs, results, num_rel_doc, non_doc, ordered_docs)
    ...
    # calc average value
    values['Precision'] = total_precision / num_queries
    values['Recall'] = total_recall / num_queries
    values['P@10'] = total_p_at_10 / num_queries
    values['R-Precision'] = total_r_precision / num_queries
    values['MAP'] = total_average_precision / num_queries
    values['bpref'] = total_bpref / num_queries
...
if __name__ == "__main__":
    ...
    if os.path.exists(results_file):
        ...
        print('Please run the search_large_corpus first')
